...
fn external_process_kill_recovery_contract() {
    ...
    for (case, expect_clean) in [
        ...
        let mut initial = RegionStore::open(
            4096,
            FileRegionBackend::for_test(directory.files.clone(), data, 4096),
        )
        .unwrap();
    ...
    match case {
        ...
        "warm-data" | "warm-image" | "clean-state" => {
            ...
            let mut store = RegionStore::open(
                4096,
                FileRegionBackend::for_test_with_file_system(files, data, 4096, file_system),
            )
            .unwrap();
...
fn configured_read_wait_is_bounded_and_cancel_safe() {
    ...
    let runtime_config = RuntimeOptions {
        io_engine: IoEngineOptions::Posix(PosixIoOptions::new(2, 4, 1)),
        l1_capacity_bytes: 0,
        statistics: true,
...
fn queued_l2_read_does_not_pin_warm_close() {
    ...
    let data = production_data_superblock(512 * 1024);
    let runtime_config = RuntimeOptions {
        io_engine: IoEngineOptions::Posix(PosixIoOptions::new(1, 4, 1)),
        l1_capacity_bytes: 0,
        read_admission: ReadAdmission::Wait {
...
fn clean_image_rebinds_a_different_append_shard_topology() {
    ...
    let mut reopened = RegionStore::open(
        config,
        FileRegionBackend::for_test(directory.files.clone(), data, config),
    )
    .unwrap();
    assert_eq!(reopened.startup(), StartupMode::Warm);
...
fn recovery_sidecars_must_share_one_directory() {
    ...
    let opened = RegionStore::open(
        8,
        FileRegionBackend::for_test(files, test_data_superblock(), 8),
    );
    assert!(matches!(
        opened,
        Err(error) if error.kind() == io::ErrorKind::InvalidInput
