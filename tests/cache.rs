...
fn data_extent_contains_nonzero(path: &std::path::Path) -> bool {
    const DATA_HEADER_BYTES: u64 = 4096;
    let mut file = std::fs::File::open(path).unwrap();
    file.seek(SeekFrom::Start(DATA_HEADER_BYTES)).unwrap();
...
#[tokio::test]
async fn posix_regions_stripe_across_data_files_and_recover_warm() {
    let primary = TestCache::new("striped-primary");
    let secondary = TestCache::new("striped-secondary");
    let builder = || {
        primary
            .config(2)
            .with_additional_data_paths([secondary.data.as_path()])
    };
    let cache = builder().open().await.unwrap();
    assert_eq!(cache.startup_mode(), StartupMode::Cold);
    for ordinal in 0_u32..64 {
        eventually_admitted(|| cache.put(ordinal.to_le_bytes(), vec![ordinal as u8 + 1; 4096]));
    }
    cache.drain().await.unwrap();
    cache.close_warm().await.unwrap();

    assert_eq!(
        std::fs::metadata(&primary.data).unwrap().len(),
        4096 + 2 * 512 * 1024
    );
    assert_eq!(
        std::fs::metadata(&secondary.data).unwrap().len(),
        4096 + 512 * 1024
    );
    assert!(data_extent_contains_nonzero(&primary.data));
    assert!(data_extent_contains_nonzero(&secondary.data));

    let reopened = builder().open().await.unwrap();
    assert_eq!(reopened.startup_mode(), StartupMode::Warm);
    for ordinal in 0_u32..64 {
        let value = reopened.get(ordinal.to_le_bytes()).await.unwrap().unwrap();
        assert_eq!(value.as_ref(), vec![ordinal as u8 + 1; 4096]);
    }
    reopened.close_warm().await.unwrap();

    std::fs::remove_file(&secondary.data).unwrap();
    let cold = builder().open().await.unwrap();
    assert_eq!(cold.startup_mode(), StartupMode::Cold);
    assert!(cold.get(0_u32.to_le_bytes()).await.unwrap().is_none());
    cold.close_fast().await.unwrap();
}

#[tokio::test]
async fn reordered_data_devices_cold_start_instead_of_trusting_the_old_image() {
    let primary = TestCache::new("reordered-primary");
    let second = TestCache::new("reordered-second");
    let third = TestCache::new("reordered-third");
    let cache = primary
        .config(1)
        .with_additional_data_paths([second.data.as_path(), third.data.as_path()])
        .open()
        .await
        .unwrap();
    cache.put("key", "value").unwrap();
    ...
    let reopened = primary
        .config(1)
        .with_additional_data_paths([third.data.as_path(), second.data.as_path()])
        .open()
        .await
        .unwrap();
    assert_eq!(reopened.startup_mode(), StartupMode::Cold);
...
#[tokio::test]
async fn every_data_device_is_exclusively_owned() {
    let first_primary = TestCache::new("device-lock-first");
    let second_primary = TestCache::new("device-lock-second");
    let shared = TestCache::new("device-lock-shared");
    let first = first_primary
        .config(1)
        .with_additional_data_paths([shared.data.as_path()])
        .open()
        .await
        .unwrap();
    let conflicting = second_primary
        .config(1)
        .with_additional_data_paths([shared.data.as_path()])
        .open()
        .await;
    assert!(conflicting.is_err());
    first.close_fast().await.unwrap();
}

#[tokio::test]
async fn duplicate_data_paths_are_rejected_before_file_creation() {
    let files = TestCache::new("duplicate-data-path");
    let error = files
        .config(1)
        .with_additional_data_paths([files.data.as_path()])
        .open()
        .await
        .unwrap_err();
...
#[tokio::test]
async fn multi_device_io_uring_is_rejected_before_file_creation() {
    let primary = TestCache::new("multi-device-io-uring-primary");
    let secondary = TestCache::new("multi-device-io-uring-secondary");
    ...
    primary.assert_absent();
    secondary.assert_absent();
}
