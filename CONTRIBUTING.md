...
`cargo x check` verifies the workspace and each optional `cache2` feature. `cargo x test` runs workspace tests with all features and the ignored extended library tests. `cargo x lint` checks Rust formatting, Clippy, public documentation, the publishable package, license headers, dependency licenses, advisories, and sources.
