...
```rust
use cache2::{CacheConfig, RuntimeConfig, StaticConfig};
...
`CacheConfig::with_tokio_handle` to bind the cache to another runtime; that
...
append shards, independent read/write I/O workers, I/O mode, write-batch size,
and optional statistics. Runtime tuning may change across opens, except that an
append-shard count different from the clean image cold-starts empty.

POSIX positioned I/O is the default production path. Buffered, automatic, and
direct modes are supported where the filesystem allows them. Linux io_uring is
an explicit `io-uring` crate feature and runtime selection.

Each append shard owns one Active Region, two Region-sized staging buffers, and
one ordered worker. Read and write workers bound separate I/O pools; increasing
append shards can raise write parallelism but has a direct fixed-memory cost.
...
needs about 7.62 GiB for the index. The aggregate memory limit must also cover
L1, two staging buffers per append shard, metadata, transient reads, and cache
thread stacks. Invalid plans fail before cache files are created.
