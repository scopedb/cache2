//! Checked Format V1 value-record encoding for the RegionStore V2 append path.
//!
//! V2 reuses the stable record envelope while keeping batch padding and
...
use crate::recovery_v2::{RECORD_ALIGNMENT_V2, REGION_HEADER_SIZE_V2};
use crate::region_manager_v2::RegionAppendReservationV2;
...
pub(crate) struct EncodedValueV2 {
    pub(crate) hash: u64,
    pub(crate) entry: IndexEntry,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum RecordEncodeErrorV2 {
    LengthOverflow,
    KeyTooLarge,
    ValueTooLarge,
    RecordTooLarge,
...
impl fmt::Display for RecordEncodeErrorV2 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::LengthOverflow => formatter.write_str("V2 record length overflow"),
            Self::KeyTooLarge => formatter.write_str("V2 record key exceeds the format limit"),
            Self::ValueTooLarge => formatter.write_str("V2 record value exceeds the format limit"),
            Self::RecordTooLarge => {
                formatter.write_str("V2 record length exceeds the packed-location limit")
            }
            Self::DestinationLengthMismatch {
                ...
                formatter,
                "V2 record destination length {destination} does not match reservation {reserved}"
            ),
            Self::ReservationTooSmall { required, reserved } => write!(
                formatter,
                "V2 record reservation {reserved} is smaller than required length {required}"
            ),
            Self::InvalidReservation => {
                formatter.write_str("V2 record reservation identity is invalid")
            }
            Self::InvalidLocation(error) => {
                write!(formatter, "invalid V2 record location: {error}")
            }
        }
    }
}

impl std::error::Error for RecordEncodeErrorV2 {
...
pub(crate) fn required_record_bytes(
    ...
    key_len: usize,
    value_len: usize,
) -> Result<u32, RecordEncodeErrorV2> {
    let encoded_key_len = encoded_key_len(namespace_id, key_len)?;
    if encoded_key_len > MAX_KEY_SIZE {
        return Err(RecordEncodeErrorV2::KeyTooLarge);
    }
    if value_len > MAX_VALUE_SIZE {
        return Err(RecordEncodeErrorV2::ValueTooLarge);
    }
    let record_bytes = RecordHeader::aligned_len(encoded_key_len, value_len)
        .ok_or(RecordEncodeErrorV2::LengthOverflow)?;
    if record_bytes > MAX_RECORD_LEN {
        return Err(RecordEncodeErrorV2::RecordTooLarge);
    }
    Ok(record_bytes)
}

/// Returns the minimum Format V1 envelope used while filling a write batch.
///
...
///
/// `reservation.record_bytes` is the minimum Format V1 envelope on the V2
...
pub(crate) fn encode_value_into(
    destination: &mut [u8],
    reservation: RegionAppendReservationV2,
    hash_seed: u64,
    namespace_id: u32,
    key: &[u8],
    value: &[u8],
    expires_at: u64,
) -> Result<EncodedValueV2, RecordEncodeErrorV2> {
    let required = required_record_bytes(namespace_id, key.len(), value.len())?;
    let destination_len = destination.len();
    let reserved_len = usize::try_from(reservation.record_bytes)
        .map_err(|_| RecordEncodeErrorV2::LengthOverflow)?;
    if destination_len != reserved_len {
        return Err(RecordEncodeErrorV2::DestinationLengthMismatch {
            reserved: reservation.record_bytes,
            destination: destination_len,
        });
    }
    if reservation.record_bytes < required {
        return Err(RecordEncodeErrorV2::ReservationTooSmall {
            required,
            reserved: reservation.record_bytes,
        });
    }
    if reservation.cache_epoch == 0
        ...
        || reservation.seqno == 0
        || reservation.offset < REGION_HEADER_SIZE_V2
        || reservation.offset % RECORD_ALIGNMENT_V2 != 0
    {
        return Err(RecordEncodeErrorV2::InvalidReservation);
    }
    ...
    .map_err(RecordEncodeErrorV2::InvalidLocation)?;
    let encoded_key_len = encoded_key_len(namespace_id, key.len())?;
    let value_len = u32::try_from(value.len()).map_err(|_| RecordEncodeErrorV2::ValueTooLarge)?;
    let key_len =
        u32::try_from(encoded_key_len).map_err(|_| RecordEncodeErrorV2::LengthOverflow)?;
...
fn encoded_key_len(namespace_id: u32, raw_key_len: usize) -> Result<usize, RecordEncodeErrorV2> {
    raw_key_len
...
pub(crate) fn hash_namespaced_key_v2(seed: u64, namespace_id: u32, key: &[u8]) -> u64 {
    let mut hash = FNV_OFFSET_BASIS ^ seed;
    if namespace_id != 0 {
        hash_update(&mut hash, NAMESPACE_HASH_DOMAIN);
        hash_update(&mut hash, &namespace_id.to_le_bytes());
    }
...
mod tests {
    ...
    fn namespaced_16k_value_uses_the_minimum_format_envelope() {
        ...
        let reservation = RegionAppendReservationV2 {
            lane_id: 0,
            cache_epoch: 5,
            region_id: 7,
        ...
        assert_eq!(encoded.hash, 0xac17_6934_85d3_3e78);
        assert_eq!(encoded.entry.location.region_id(), 7);
        assert_eq!(encoded.entry.location.offset(), REGION_HEADER_SIZE_V2);
    ...
    fn target_chunk_sizes_do_not_pay_per_record_direct_io_padding() {
        let mut cursor = usize::try_from(REGION_HEADER_SIZE_V2).unwrap();
        ...
        for (namespace_id, key_len, value_len) in [
            ...
            assert_eq!(planned, required);
            assert_eq!(planned % RECORD_ALIGNMENT_V2, 0);
    ...
    fn invalid_lengths_and_reservations_return_typed_errors_without_writes() {
        assert_eq!(
            ...
            Err(RecordEncodeErrorV2::KeyTooLarge)
        );
        assert_eq!(
            required_record_bytes(0, 1, MAX_VALUE_SIZE + 1),
            Err(RecordEncodeErrorV2::ValueTooLarge)
        );
        ...
        assert!(matches!(
            ...
            Err(RecordEncodeErrorV2::DestinationLengthMismatch { .. })
        ));
