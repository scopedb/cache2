...
use cache_rs::{HybridCacheConfig, StaticConfig};
...
async fn main() -> io::Result<()> {
    ...
    let static_config = StaticConfig::new(5 * 4096).with_region_size(4096);
    let cache = HybridCacheConfig::from_static(path, static_config)
        .open()
        .await?;
